...
(use-package projectile
  ...
  (setq projectile-project-root-files-bottom-up
        '(".projectile" ".git" ".hg" ".svn" "CVS" "Makefile" "CMakeLists.txt" 
...
;; Note: Use standard Emacs bindings (C-c p ...) for navigation
;; Evil is for editing, not navigation
(with-eval-after-load 'projectile
  (define-key projectile-mode-map (kbd "C-c p b") 'consult-project-buffer))
#+end_src
...
(use-package dired-hide-dotfiles
  ...
  ;; Evil mode binding (if using evil-mode without evil-collection)
  (with-eval-after-load 'evil
    (evil-define-key 'normal dired-mode-map (kbd ".") 'dired-hide-dotfiles-mode))
  
  ;; Alternative: if using evil-collection, use this instead
...
(use-package dumb-jump
  ...
  ;; Don't be aggressive by default (can be noisy in comments/strings)
  ...
  (with-eval-after-load 'evil
    ...
    ;; Also bind M-. and M-, to work in all Evil states
    (dolist (state '(normal insert visual motion emacs))
...
;; Bind to M-s h (Alt+s h)  for quick system header search
...
;; C-M-s → Search /usr/include system headers
#+end_src
