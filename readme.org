...
;; Set font
(set-face-attribute 'default nil :font "CaskaydiaMono Nerd Font Mono" :weight'semilight :height 200)
...
;; 1. The modern standard for smooth scrolling (Emacs 29+)
(pixel-scroll-precision-mode 1)

;; 2. Keep cursor at the same screen position
(setq scroll-preserve-screen-position t)

;; 3. Prevent the "jump" when cursor hits the edge
...
;; Separate custom.el from init.el (prevents auto-appended junk)
...
(defun my/dired-setup-consult-keys ()
  ...
  (define-key dired-mode-map (kbd "M-s g") #'consult-grep)
...
(use-package minibuffer
  ...
  :config
  ;; Smarter C-g - closes minibuffer if open, quits region if inside it
  (define-advice keyboard-quit
      (:around (quit) quit-current-context)
    "Quit the current context.
Closes minibuffer if active and we are outside it, uses
`minibuffer-keyboard-quit' if inside it, or calls `keyboard-quit'
otherwise."
  ...
  ;; Keep cursor out of read-only prompt area
...
(use-package cape
  :ensure t
  :init
  ;; Add cape backends to corfu globally
  ...
  :config
  (setq cape-dabbrev-check-other-buffers t   ;; scan all buffers for words
        cape-dabbrev-min-length          3)) ;; only complete words 3+ chars
#+end_src
* Compilation
#+begin_src emacs-lisp
...
(defvar woman-topic-history nil
  "History for WoMan topic prompts.")
...
(defun my/get-system-search-paths ()
  "Return a list of relevant system paths based on current major mode."
  (pcase major-mode
    ((or 'c-mode 'c-ts-mode 'c++-mode 'c++-ts-mode)
    ...
    ((or 'go-mode 'go-ts-mode)
     (seq-filter #'file-directory-p
                 (list
                  (when (getenv "GOROOT")
                    (expand-file-name "src" (getenv "GOROOT")))
                  "/usr/local/go/src"
                  "/usr/lib/go/src")))
    ((or 'python-mode 'python-ts-mode)
     (seq-filter #'file-directory-p
                 (append
                  (list "/usr/lib/python3"
                        "/usr/local/lib/python3")
                  (file-expand-wildcards "/usr/lib/python3.*")
                  (file-expand-wildcards "/usr/local/lib/python3.*"))))
    (_
     (seq-filter #'file-directory-p
                 (list "/usr/include"
                       "/usr/local/include"
...
(defun my/search-system-headers ()
  "Search for symbol at point in system headers/stdlib based on current major mode.
Supports C (/usr/include), Go (GOROOT), and Python (stdlib site-packages)."
  (interactive)
  (let* ((symbol (thing-at-point 'symbol t))
         (search-paths (my/get-system-search-paths))
         (path (if (length> search-paths 1)
                   (completing-read
                    (format "Search '%s' in: " symbol)
                    search-paths nil t)
                 (car search-paths))))
    (if (and path (file-directory-p path))
        (let ((dumb-jump-default-project path)
              (dumb-jump-project-denoters nil))
          (message "Searching '%s' in %s..." symbol path)
          (dumb-jump-go))
      (user-error "No valid search path found for %s. Path: %s"
                  major-mode path))))
...
(use-package yasnippet
  ...
  (setq yas-snippet-dirs
        '("~/.emacs.d/snippets"                 ;; check here for your custom snippets
          ))
  (yas-global-mode 1))                          ;; Loads yasnippet in all modes

(use-package yasnippet-snippets
  :ensure t
  :after yasnippet)                             ;; Provides the actual "library" of snippets
#+end_src
* Debugging with Dape
#+begin_src emacs-lisp
(use-package dape
...
#+end_src
** Multi-vterm (Named Terminal Sessions)
#+begin_src emacs-lisp
(use-package multi-vterm
  ...
  ;; Switch to an existing named session or create it
  (defun my/multi-vterm-switch (name)
    "Switch to named vterm session NAME, creating it if needed.
The buffer is named *vterm: NAME*."
    ...
    (let ((buf-name (format "*vterm: %s*" name)))
      (if-let ((buf (get-buffer buf-name)))
        ...
        (message "Created vterm session: %s" name))))
 
  ;; Pick from existing sessions or create a new one
  (defun my/multi-vterm-consult ()
    "Select a named vterm session with consult, or create a new one."
    (interactive)
    (require 'consult)
    (let* ((sessions (mapcar #'buffer-name
                             (seq-filter
                              (lambda (b)
                              ...
                              (buffer-list))))
           (choice (consult--read
                    (append sessions '("[new session]"))
                    :prompt "vterm session: "
                    :require-match t
                    :sort nil
                    :category 'buffer)))
      (if (equal choice "[new session]")
          (call-interactively #'my/multi-vterm-switch)
...
;; 2. Python: Use the interpreter for docs (No LSP)
(use-package python
  :ensure nil
  :config
  (setq python-eldoc-get-doc-strategy 'python-eldoc-format-all-docs))

;; 3. Go: Third-party bridge for signatures
(use-package go-eldoc
  :ensure t ; Ensure this is installed!
